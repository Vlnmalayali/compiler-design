...
class Item:
    def __init__(self, production, dot_position, lookahead):
        self.production = (production[0], tuple(production[1]))  # Convert list to tuple for hashing
            ...
            if sym == nonterminal:
                if idx + 1 < len(prod[1]):
                    ...
                    if 'ε' in first_cache[next_symbol]:
                        follow_set.update(follow(prod[0], grammar, first_cache, follow_cache))
                else:
...
def goto(items, symbol, grammar, first_cache):
    goto_set = set()
    for item in items:
        if item.dot_position < len(item.production[1]) and item.production[1][item.dot_position] == symbol:
        ...
        for item in state:
            if item.dot_position == len(item.production[1]):
                if item.production[0] == grammar.start_symbol:
                    ...
                    for follow_symbol in follow(item.production[0], grammar, first_cache, follow_cache):
